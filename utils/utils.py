...
def make_env(env_id, seed):
    ...
    if env_id == 'ball_in_cup_catch':
        ...
        if task_name not in suite._DOMAINS[domain_name].SUITE.keys():
            if domain_name not in default_task.keys():
                raise ValueError("Task {} not found in domain {}".format(task_name, domain_name))
            task_name = default_task[domain_name]
    ...
    assert env.action_space.low.min() >= -1 # type: ignore
    assert env.action_space.high.max() <= 1 # type: ignore
...
def local_timestr(timestamp, format='%Y-%m-%d %H:%M:%S'):
    dt_local = datetime.fromtimestamp(timestamp)
    formatted_local = dt_local.strftime(format)
