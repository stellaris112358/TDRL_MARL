...
class MADDPG:
    ...
    def train(self, transitions, other_agents):
        for key, value in transitions.items():
            # torch.as_tensor avoids extra copies and silences the PyTorch warning
            if isinstance(value, torch.Tensor):
                transitions[key] = value.to(dtype=torch.float32, device=self.device)
            else:
                transitions[key] = torch.as_tensor(value, dtype=torch.float32, device=self.device)
