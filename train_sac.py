...
class Workspace:
    def __init__(self, cfg):
        ...
        self.video_recoder = VideoRecorder(Path.cwd() if cfg.save_video else None)
        self.step = 0
    ...
    def evaluate(self):
        ...
        for episode in range(num_eval_episodes):
            ...
            if is_record_video:
                self.video_recoder.init(self.env)
            ...
            while not done:
                ...
                if is_record_video:
                    self.video_recoder.record(self.env)

            average_episode_reward += episode_reward
            if self.log_success:
                success_rate += episode_success
            if is_record_video:
                self.video_recoder.save(f'{self.step}.mp4')
        ...
        while self.step < self.cfg.num_train_steps:
            if done:
                ...
                self.logger.log('train/episode_reward', episode_reward,
                                self.step)
                self.logger.log('train/true_episode_reward', episode_reward,
                                self.step)
...
@hydra.main(config_path='conf', config_name='sac', version_base=None)
